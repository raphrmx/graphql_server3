...
final GraphQLEnumType<String> _directiveLocationType = enumTypeFromStrings(
  '__DirectiveLocation',
  DirectiveLocation.values.map((v) => _screamingSnakeCase(v.name)).toList(),
);
    ...
    fields: [
      ...
      field(
        ...
        resolve: (obj, _) => (obj as GraphQLDirectiveType).locations
            .map((v) => _screamingSnakeCase(v.name))
            .toList(),
      ),
      field(
        'args',
...
/// Renders an identifier the way GraphQL spells enum values.
///
/// A camel or pascal cased name gains an underscore at each word boundary and
/// is upper cased, so `inProgress` becomes `IN_PROGRESS`. Spaces and hyphens
/// count as boundaries too, and a name that is already upper cased is left as
/// it stands.
///
/// ### Example:
/// ```dart
/// _screamingSnakeCase('inProgress'); // IN_PROGRESS
/// ```
String _screamingSnakeCase(String name) => name
    .replaceAllMapped(RegExp(r'(?<=[a-z0-9])(?=[A-Z])'), (Match _) => '_')
